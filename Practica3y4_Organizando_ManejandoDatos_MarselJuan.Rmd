...
gather(pew_religion,key= "income", value = "freq",-religion, factor_key=T)
...
```{r}
pivot_longer(raw, names_to = "week", values_to = "rank", cols = starts_with("wk"), names_prefix = "wk")
```
...
gather(raw, key = "code",value = "cases",-country,-year) %>%
  separate(col = code, into = c("sex","age"), sep = 1)
```
...
```{r}
pivot_longer(raw, names_to = "dia", values_to ="value", cols = starts_with("d"), names_prefix = "d", values_drop_na = TRUE ) %>%
  pivot_wider(names_from = element, values_from = value) %>%
  mutate(fecha = make_date(year,month,dia))
...
planes.f <- planes %>%
  filter(manufacturer != "MCDONELL DOUGLAS", seats>90 & seats<120) %>% 
  mutate(manufacturer = ifelse(manufacturer=="AIRBUS","AIRBUS COMP",manufacturer))
...
```{r }
flights<-flights %>% select(!ends_with("time"), -"time_hour")
planes<-planes %>% select(!c(speed,type))
```
...
```{r }
flights %>% mutate("Full_Date" = paste(day, month, year.x, sep ="-")) %>% select(-day,-month,-year.x)
